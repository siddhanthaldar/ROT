...
class RobotHangHangerEnv(robot_env.RobotEnv):
	def __init__(self, height=84, width=84, step_size=10, enable_arm=True, enable_gripper=True, enable_camera=True, camera_view='side',
				 use_depth=False, dist_threshold=0.05, random_start=True, x_limit=None, y_limit=None, z_limit=None, pitch=-90, roll=180, keep_gripper_closed=True, start_at_the_back=True):
		robot_env.RobotEnv.__init__(
			...
			pitch=pitch,
			roll=roll,
			start_at_the_back=start_at_the_back
		)
	...
	def get_random_pos(self, pos=None):
		...
		pos[0] = x_disp if self.start_at_the_back else self.x_limit[0]
		pos[1] = y_disp 
